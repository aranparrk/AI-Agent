...
-- 열 이름에 별칭 부여하기 : 아이디, 이름, 멤버 수, 전화번호, 데뷔일자
SELECT mem_id AS 아이디, mem_name AS 이름,  mem_number AS "멤버 수", CONCAT(phone1, phone2) AS 전화번호, debut_date AS 데뷔일
FROM member; 

-- 아이디, 이름, 주소, 키 별칭으로 출력하기
SELECT mem_id AS 아이디, mem_name AS 이름, addr AS 주소, height AS 키 FROM member; 

-- 정렬 및 제한 : 키가 큰 멤버 5명만 출력하기
SELECT * FROM member
