# 딕셔너리 : 별도의 키를 통해 각 요소를 접근할 수 있도록 만들어진 데이터 타입
...
print(coffee_menu)
print(coffee_menu['Espresso']) # 키로 값 확인
print(coffee_menu.get('Latte')) # 키로 값 확인
...
coffee_menu.update({'Americano' : 3000, 'Espresso' : 3000, 'Latte' : 4500, 'Moca' : 5000, '스무디' : 5500})
...
# 문제 2. 값 조회
# coffe_menu에서 "Moca"의 가격을 get() 함수를 이용해 출력하세요. 만약 존재하지 않는 메뉴 ('Cappuccino')를
# get()으로 조회하면 어떻게 출력되는지도 확인해보세요.
print(coffee_menu.get('Moca'))
print(coffee_menu.get('Cappuccino'))

# 문제 3. 추가와 삭제
# coffe_menu에 'Cappuccino' : 4800을 새로 추가하고, 'Moca'를 삭제한 뒤 결과를 출력하세요.
coffee_menu['Cappuccino'] = 4000
...
for i in coffee_menu:
    if coffee_menu[i] >= 4000:
        print(f'{i} - {coffee_menu[i]}')
...
for i in coffee_menu:
    coffee_price[i] = int(coffee_menu[i] * 1.1)

coffee_menu.update(coffee_price)
